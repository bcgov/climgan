...
worldclim_folder <- "C:/Users/TGRICE/OneDrive - Government of BC/Documents/GANs/WorldClim/tmax/mar/"
...
worldclim <- rast(paste(worldclim_folder, "worldclim_coarse.nc", sep = ""))
mask <- rast(paste(worldclim_folder, "worldclim_mask_train.nc", sep = ""))
...
worldclim_folder = r.worldclim_folder
...
## WorldClim
worldclim_fields = xr.open_dataset(worldclim_folder + "worldclim_coarse.nc")

# chop a slice of Alaska ERA5 off and save as testing data
worldclim_cropped, worldclim_test = slice_map(worldclim_fields, 24)

# standardize
standardized_worldclim, standardized_worldclim_test, worldclim_mean, worldclim_std = standardize(worldclim_cropped, worldclim_test)

plt.close()
plt.imshow(standardized_worldclim)
plt.show()

plt.close()
plt.imshow(standardized_worldclim_test)
plt.show()
...
def remove_tiles(hrcov, lrcov, dem):
  assert hrcov.size(0) == lrcov.size(0) == dem.size(0), "Tensors must be same size"
  # nan_mask = torch.isnan(hrcov) | torch.isnan(lrcov)
  # bad_tiles = nan_mask.view(hrcov.size(0), -1).any(dim=1)
  hrcov_nan = torch.isnan(hrcov).view(hrcov.size(0), -1).any(dim=1)
  lrcov_nan = torch.isnan(lrcov).view(lrcov.size(0), -1).any(dim=1)
  bad_tiles = hrcov_nan | lrcov_nan
  clean_hrcov = hrcov[~bad_tiles]
  clean_lrcov = lrcov[~bad_tiles]
  clean_dem = dem[~bad_tiles]
  return clean_hrcov, clean_lrcov, clean_dem

def remove_zero_tiles(hrcov, lrcov, dem):
  assert hrcov.size(0) == lrcov.size(0) == dem.size(0), "Tensors must be same size"
  zero_mask = (hrcov == 0).view(hrcov.size(0), -1).any(dim=1)
  clean_hrcov = hrcov[~zero_mask]
  clean_lrcov = lrcov[~zero_mask]
...
def add_rotations(tiles):
  """ rotates each tile 90, 180, 270 degrees """
  rotations_lists = [[] for _ in range(4)]  # 0°, 90°, 180°, 270°
  
  for row in tiles:
      for tile in row:
          rotations_lists[0].append(tile)                               # original
          rotations_lists[1].append(torch.rot90(tile, 1, dims=(-2, -1)))  # 90°
          rotations_lists[2].append(torch.rot90(tile, 2, dims=(-2, -1)))  # 180°
          rotations_lists[3].append(torch.rot90(tile, 3, dims=(-2, -1)))  # 270°
          
  return rotations_lists
...
# era5_tiles_rotated = add_rotations(era5_tiles)
# era5_test_tiles_rotated = add_rotations(era5_test_tiles)

## WorldClim
worldclim_tiles = tile_data(standardized_worldclim.unsqueeze(0), int(tile_size / scale_factor), int(offset / scale_factor))
worldclim_test_tiles = tile_data(standardized_worldclim_test.unsqueeze(0), int(tile_size / scale_factor), int(offset / scale_factor))
...
plt.close()
plt.imshow(worldclim_tiles[4][8][0,...]) # mar
plt.show()
...
worldclim_tiles_flat = torch.cat([tile for row in worldclim_tiles for tile in row], dim=0)
...
worldclim_tiles_flat_test = torch.cat([tile for row in worldclim_test_tiles for tile in row], dim=0)
...
clean_prism, clean_worldclim, clean_dem = remove_tiles(prism_tiles_flat, worldclim_tiles_flat, dem_tiles_flat)

# make sure number of tiles is a multiple of batch size = 10
clean_prism = trim_tiles(clean_prism, 10)
clean_worldclim = trim_tiles(clean_worldclim, 10)
clean_dem = trim_tiles(clean_dem, 10)

# remove any tiles with missing values for testing
clean_prism_test, clean_worldclim_test, clean_dem_test = remove_tiles(prism_tiles_flat_test, worldclim_tiles_flat_test, dem_tiles_flat_test)

# remove any tiles that have any values=0 for testing
clean_prism_test, clean_worldclim_test, clean_dem_test = remove_zero_tiles(clean_prism_test, clean_worldclim_test, clean_dem_test)

# check shapes
print(clean_prism.shape)
print(clean_worldclim.shape)
print(clean_dem.shape)
print(clean_prism_test.shape)
print(clean_worldclim_test.shape)
print(clean_dem_test.shape)

torch.save(clean_prism, prism_folder + "prism_tiles.pt")
torch.save(clean_worldclim, worldclim_folder + "worldclim_tiles.pt")
...
torch.save(clean_worldclim_test, worldclim_folder + "worldclim_test_tiles.pt")
...
```{python}
final_pad = torch.load("O:/Mosaic_Yukon/Tirion/Results/Run7/GAN_gen250/Run7_gen250_fullregion_sd.pt")
...
results_folder <- "O:/Mosaic_Yukon/Tirion/Results/Run7/GAN_gen250/"

data_folder <- "C:/Users/TGRICE/OneDrive - Government of BC/Documents/GANs/PRISM/tmax/mar/"
...
mask_cropped <- crop(mask, res,  snap="in")
mask_aligned <- resample(mask_cropped, res, method = "near")
final <- mask(res, mask_aligned, maskvalues=0)
plot(final)

writeCDF(final, paste0(results_folder, "Run7_gen250_fullregionsd_masked.nc"), varname='tmax', overwrite = T)
```

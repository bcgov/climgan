...
dem_test_tiles = tile_data(standardized_dem.unsqueeze(0), tile_size, offset)
...
for num in range(12):
  ...
  plt.close()
  plt.imshow(era5_list_tiles[num][2][4][num,...])
  plt.show()
...
plt.close()
plt.imshow(era5_test_tiles[2][4][0,...])
plt.show()
...
```{python}
final_pad = torch.load("O:/Mosaic_Yukon/Tirion/Results/Run12/mar/mar_gen250_fullregion.pt")
...
results_folder <- "O:/Mosaic_Yukon/Tirion/Results/Run12/mar/"
...
writeCDF(final, paste0(results_folder, "mar_fullregion_masked.nc"), varname='tmax', overwrite = T)
```

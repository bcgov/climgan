...
dem <- rast(paste(dem_folder, "dem_NWNA_coarse_test.nc", sep = ""))
...
era5 <- rast(paste(era5_folder, "tasmax_1981_2010_cropped_test.nc", sep = ""))
...
prism_list <- lapply(months, function(m) {
  rast(file.path(prism_folder, m, "prism_train_coarse_test.nc"))
})
...
writeCDF(land_mask, paste0(dem_folder, "dem_mask_test.nc"), varname='tmax', overwrite = T)
```
...
## ERA5
era5_fields = xr.open_dataset(era5_folder + "tasmax_1981_2010_cropped_test.nc")
...
for m in months:
  prism_fields = xr.open_dataset(prism_folder + "/" + m +"/prism_train_coarse_test.nc")
  prism = torch.from_numpy(prism_fields.to_array().to_numpy())[0,...]
  prism_list.append(prism)
  
# prism_fields = xr.open_dataset(prism_folder + "prism_train_coarse_test.nc")
# prism = torch.from_numpy(prism_fields.to_array().to_numpy())[0,...]
...
## DEM
dem_fields = xr.open_dataset(dem_folder + "dem_NWNA_coarse_test.nc")
...
## ERA5
era5_tiles = tile_data(standardized_era5[0:12,...], int(tile_size / scale_factor), int(offset / scale_factor))
# era5_test_tiles = tile_data(standardized_era5_test[[11,12,13],...], int(tile_size / scale_factor), int(offset / scale_factor))
...
for num in range(12):
  plt.close()
  plt.imshow(prism_list_tiles[num][2][4][0,...])
  plt.show()

  plt.close()
  plt.imshow(era5_tiles[2][4][num,...])
  plt.show()
...
flattened_months = []
for num in range(12):
  flat = torch.cat([tile for row in prism_list_tiles[num] for tile in row], dim=0)
  flattened_months.append(flat)
  
prism_tiles_flat = torch.stack(flattened_months, dim=1)
#prism_tiles_flat = torch.cat([tile for row in prism_list_tiles[num] for tile in row], dim=0)
...
```{python}
final_pad = torch.load("O:/Mosaic_Yukon/Tirion/Results/Run9/tmax/aug/aug_gen150_fullregion.pt")
...
results_folder <- "O:/Mosaic_Yukon/Tirion/Results/Run9/tmax/aug/"

data_folder <- "C:/Users/TGRICE/OneDrive - Government of BC/Documents/GANs/PRISM/tmax/aug/"
...
writeCDF(final, paste0(results_folder, "aug_fullregion_masked.nc"), varname='tmax', overwrite = T)
```

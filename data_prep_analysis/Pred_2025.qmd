...
```{python}
def slice_map(dataset, num_pixels):
...
def fill_tile_nans_with_mean(tiles):
    return torch.stack([
        torch.where(
            torch.isnan(tile),
...
clean_prism_test = fill_tile_nans_with_mean(prism_tiles_flat_test)
clean_era5_test = fill_tile_nans_with_mean(era5_tiles_flat_test)
clean_dem_test = fill_tile_nans_with_mean(dem_tiles_flat_test)

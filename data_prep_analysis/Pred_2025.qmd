...
```{python}
def slice_map(dataset, num_pixels, save_path_train=None, save_path_test=None):
  ...
  if save_path_train:
        array_cropped.to_netcdf(save_path_train)
  if save_path_test:
        test_array.to_netcdf(save_path_test)
...
era5_cropped, era5_test = slice_map(era5_fields, 24)
...
plt.close()
plt.imshow(standardized_era5_test[3]) # by month
plt.show()
...
prism_cropped, prism_test = slice_map(prism_fields, 288, prism_folder + "prism_train_cropped.nc", prism_folder + "prism_test_cropped.nc")
...
def na_to_mean(tiles):
  """ only pass in a tensor in shape [numtiles, dim, dim] """
    return torch.stack([
        torch.where(
            torch.isnan(tile),
            torch.full_like(tile, tile[~torch.isnan(tile)].mean() if torch.any(~torch.isnan(tile)) else 0.0),
            tile
        )
        for tile in tiles
    ])
...
## ERA5
# this is for using previous 2 months as covariates
# months = [1,2,3]
# tiles_per_month = []
# 
# for m in months:
#     tiles = tile_data(standardized_era5[m].unsqueeze(0), int(tile_size / scale_factor), int(offset / scale_factor))
#     
#     # Flatten tile list-of-lists
#     flat_tiled = [tile for row in tiles for tile in row]
#     tiles_per_month.append(flat_tiled)
#     
# era5_tiles = torch.stack([
#     torch.cat([tiles_per_month[m][i] for m in range(3)], dim=0)
...
era5_test_tiles = tile_data(standardized_era5_test[3].unsqueeze(0), int(tile_size / scale_factor), int(offset / scale_factor))

...
## ERA5
era5_tiles = tile_data(standardized_era5[[0,1,2,12,13],...], int(tile_size / scale_factor), int(offset / scale_factor))
era5_test_tiles = tile_data(standardized_era5_test[[0,1,2,12,13],...], int(tile_size / scale_factor), int(offset / scale_factor))
...
plt.close()
plt.imshow(era5_tiles[4][8][0,...]) # jan
plt.show()
plt.close()
plt.imshow(era5_tiles[4][8][1,...]) # feb
plt.show()
plt.close()
plt.imshow(era5_tiles[4][8][2,...]) # mar
plt.show()
plt.close()
plt.imshow(era5_tiles[4][8][3,...]) # lat
plt.show()
plt.close()
plt.imshow(era5_tiles[4][8][4,...]) # lon
plt.show()
...
# need to use stack here instead of cat because there's 5 channels
...
```{python}
final_pad = torch.load("O:/Mosaic_Yukon/Tirion/Results/Run4/GAN_gen250/Run3_gen250_fullregion.pt")
...
writeCDF(res, paste0(results_folder, "Run4_gen250_fullregion.nc"), varname='tmax', overwrite = T)
```

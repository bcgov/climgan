...
```{python}
def slice_map(dataset, num_pixels, save_path_train=None, save_path_test=None, include_coords=False):
  ...
  # save the cropped maps for later comparison if needed
  if save_path_train:
        torch.save(cropped, save_path_train)
  if save_path_test:
        torch.save(test, save_path_test)
...
era5_cropped, era5_test = slice_map(era5_fields, 24, era5_folder + "era5_tasmax_train_cropped.pt", era5_folder + "era5_tasmax_test_cropped.pt", True)
...
prism_cropped, prism_test = slice_map(prism_fields, 288, prism_folder + "prism_train_cropped.pt", prism_folder + "prism_test_cropped.pt")
...
dem_cropped, dem_test = slice_map(dem_fields, 288, dem_folder + "dem_train_cropped.pt", dem_folder + "dem_test_cropped.pt")
...
def na_to_mean(tiles):
    """Only pass in a tensor in shape [numtiles, dim, dim]"""
    return torch.stack([
        torch.where(
            torch.isnan(tile),
            torch.full_like(
                tile,
                tile[~torch.isnan(tile)].mean() if torch.any(~torch.isnan(tile)) else 0.0
            ),
            tile
        )
        for tile in tiles
...
def add_covariates_era5(months, tensor):
  """ add specified months and lat/long layers to tensor """
  tiles_per_month = []

  for m in months:
      tiles = tile_data(tensor[m].unsqueeze(0), int(tile_size / scale_factor), int(offset / scale_factor))
      
      plt.close()
      plt.imshow(tiles[4][8][0,...])
      plt.show()

      # Flatten tile list-of-lists
      flat_tiled = [tile for row in tiles for tile in row]
      tiles_per_month.append(flat_tiled)

  tiles = torch.stack([
      torch.cat([tiles_per_month[m][i] for m in range(len(months))], dim=0)
      for i in range(len(tiles_per_month[0]))
])
  return tiles
...
## ERA5
# era5_tiles = tile_data(standardized_era5[2].unsqueeze(0), int(tile_size / scale_factor), int(offset / scale_factor))
# era5_test_tiles = tile_data(standardized_era5_test[2].unsqueeze(0), int(tile_size / scale_factor), int(offset / scale_factor))

era5_tiles = add_covariates_era5([0,1,2,12,13], standardized_era5)
era5_test_tiles = add_covariates_era5([0,1,2,12,13], standardized_era5_test)
...
# era5_tiles_flat_test = torch.cat([tile for row in era5_test_tiles for tile in row], dim=0)
era5_tiles_flat_test = era5_test_tiles
...
# # replace any NAs in testing tiles with tile mean
# clean_prism_test = na_to_mean(prism_tiles_flat_test)
# clean_era5_test = na_to_mean(era5_tiles_flat_test)
# clean_dem_test = na_to_mean(dem_tiles_flat_test)
...
```{python}
final_pad = torch.load("O:/Mosaic_Yukon/Tirion/Results/Run2/alaska_full_region_tmax_mar_generator_250_sd.pt")

res_np = np.array(final_pad)

plt.close()
plt.imshow(res_np, cmap='RdBu')
plt.show()
...
results_folder <- "O:/Mosaic_Yukon/Tirion/Results/Run2/"
...
values(res) <- preds
plot(res, col = gray.colors(length(values(res))))

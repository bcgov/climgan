...
# }
# gan <- list()
# gens <- c(50,100,150,200,250)
# for (i in seq_along(gens)) {
#   pred <- rast(paste0("C:/Users/TGRICE/OneDrive - Government of BC/Documents/GANs/Tirion/Results/foundational_model/tmax/Model4/jun/spec1/", "gen", gens[i], "/jun_fullregion_masked.nc"))
#   # pred.log <- log2(pred)
#   gan[[i]] <- pred
# }
gan2 <- list()
for (i in 1:12) {
  pred <- rast(paste0("C:/Users/TGRICE/OneDrive - Government of BC/Documents/GANs/Tirion/Results/foundational_model/tmax/Model4/", months[i], "/", months[i], "_fullregion_masked.nc"))
  # pred.log <- log2(pred)
  gan2[[i]] <- pred
}

for (i in 1:12) {
  us[[i]] <- project(us[[i]], gan2[[i]])
}

# Color Scheme
combined <- c(unlist(lapply(us, values)), unlist(lapply(gan2, values)))
...
map <- leaflet() %>%
  ...
  # addRasterImage(gan[[1]], colors = ColPal.raster, opacity = 1, maxBytes = 7 * 1024 * 1024, group = "Jun GAN50") %>%
  # addRasterImage(gan[[2]], colors = ColPal.raster, opacity = 1, maxBytes = 7 * 1024 * 1024, group = "Jun GAN100") %>%
  # addRasterImage(gan[[3]], colors = ColPal.raster, opacity = 1, maxBytes = 7 * 1024 * 1024, group = "Jun GAN150") %>%
  # addRasterImage(gan[[4]], colors = ColPal.raster, opacity = 1, maxBytes = 7 * 1024 * 1024, group = "Jun GAN200") %>%
  # addRasterImage(gan[[5]], colors = ColPal.raster, opacity = 1, maxBytes = 7 * 1024 * 1024, group = "Jun GAN250") %>%
  ...
  addLayersControl(
    ...
    overlayGroups = c("Jan PRISM", "Feb PRISM", "Mar PRISM", "Apr PRISM", "May PRISM",
                      "Jun PRISM","Jul PRISM", "Aug PRISM", "Sep PRISM", "Oct PRISM", "Nov PRISM", "Dec PRISM",
                      ...
                      # "Jun GAN FM4", "Jun GAN50", "Jun GAN100", "Jun GAN150", "Jun GAN200", "Jun GAN250"),
...
pred_jun_fm4_spec <- rast("C:/Users/TGRICE/OneDrive - Government of BC/Documents/GANs/Tirion/Results/foundational_model/prec/Model4/jun/spec1/gen180/jun_fullregion_masked.nc")
pred_jun_fm4_debias <- rast("C:/Users/TGRICE/OneDrive - Government of BC/Documents/GANs/Tirion/Results/foundational_model/prec/Model4/jun/spec1/debias/jun_fullregion_ext_masked.nc")
...
map <- leaflet(stn.info) %>%
  ...
  addRasterImage(pred_jun_fm4_spec, colors = ColPal.raster, opacity = 1, maxBytes = 12890669, group = "JUN GAN FM4 S1") %>%
  addRasterImage(pred_jun_fm4_debias, colors = ColPal.raster, opacity = 1, maxBytes = 12890669, group = "JUN GAN FM4 DEBIAS") %>%

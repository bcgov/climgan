...
for(e in 1:length(elements)){
  ...
  for(m in 1:12){
    ...
    temp <- rast(paste("C:/Users/CMAHONY/OneDrive - Government of BC/Data/foundational_model/", elements[e], "/Model4/", tolower(month.abb[m]), "/", tolower(month.abb[m]), "_fullregion_masked.nc", sep=""))
    if(e==3) temp <- expm1(temp) # TEMPORARY UNTIL TIRION PRODUCES RESULTS IN MM/MONTH.
    ...
    temp <- rast(paste("C:/Users/CMAHONY/OneDrive - Government of BC/Data/foundational_model/", elements[e], "/Model4/", tolower(month.abb[m]), "/spec1/gen",epoch,"/", tolower(month.abb[m]), "_fullregion_masked.nc", sep=""))
    if(e==3) temp <- expm1(temp) # TEMPORARY UNTIL TIRION PRODUCES RESULTS IN MM/MONTH.
...
for(e in 1:length(elements)){
  ...
  for(stn in stns){
    ...
    plot(stn.data, type="l", ylim=range(c(stn.data, v2024, fm4, fm4s)), lwd=2, xlab="", xaxt="n", ylab=element.names[e], main=paste("Station:", stn))
    axis(1, at=1:12, labels = month.abb, las=2, tck=0)
    lines(fm4s, col="orange")
    lines(fm4, col="dodgerblue")
    lines(v2024, col="gray", lty=2)
    legend("topleft", legend=c("Station data", "fm4s", "fm4", "v2024"), lwd=c(2,1,1,1), lty=c(1,1,1,2), col=c("black", "orange", "dodgerblue", "gray"), bty="n")

...
```{r}
# load data
test_metrics <- read.csv("O:/Mosaic_Yukon/Tirion/Results/foundational_model/Model1/mar/specialization1/test_metrics.csv")
...
```{python}
pred_dir = 'O:/Mosaic_Yukon/Tirion/Results/foundational_model/prec/Model1/apr/'
pred_path = pred_dir + 'apr_fullregion_masked.nc'
truth_dir = 'C:/Users/TGRICE/OneDrive - Government of BC/Documents/GANs/PRISM/prec/apr/'
...
pred_np = np.array(pred.prec)
truth_np = np.array(truth.prec)
...
pc1 = ax[0].pcolormesh(truth.longitude, truth.latitude, truth.prec, vmin=minmin, vmax=maxmax, cmap='viridis', label=r'$m$')
ax[0].set_title(f'truth', fontsize='medium', loc='center')
#ax[0].set_ylim(59, 71)

pc2 = ax[1].pcolormesh(pred.longitude, pred.latitude, pred.prec, vmin=minmin, vmax=maxmax, cmap='viridis', label=r'$m$')
ax[1].set_title(f'prediction', fontsize='medium', loc='center')
#ax[1].set_ylim(59, 71)

pc3 = ax[2].pcolormesh(pred.longitude, pred.latitude, truth.prec.values-pred.prec.values, vmin=dmin, vmax=dmax, cmap='RdBu', label=r'$m$')
...
fig.suptitle(f"Apr prec, MAE = %.2f" %MAE)
...
plt.savefig(pred_dir + "pred_PRISM_diff.png", dpi = 400)
plt.close()
```
...
# final_pad = torch.load("O:/Mosaic_Yukon/Tirion/Results/Run7/GAN_gen250/Run7_gen250_fullregion.pt")
# 
# # unstandardize
# mean = stand['mean'][0]
# std = stand['std'][0]
# final_pad = (final_pad*std)+mean
# 
# res_np = np.array(final_pad)
pred_np = np.array(pred.prec)
pred_np = np.log1p(pred_np)
plt.close()
fig, ax = plt.subplots()
im = ax.imshow(pred_np, cmap = 'RdYlBu_r')
# rect = patches.Rectangle((0, 0), 288, pred_np.shape[0], linewidth=2, edgecolor='black', facecolor='none')
# ax.add_patch(rect)
...
ax.axis('off')
#plt.savefig(pred_dir + "apr_full_map_masked.png", bbox_inches='tight', dpi=600)
plt.show()
...
results_folder <- "O:/Mosaic_Yukon/Tirion/Results/foundational_model/tmax/Model1/jan/"

preds <- rast(paste(results_folder, "dec_fullregion_masked.nc", sep = ""))
res <- dem[[1]]
rast_dim <- dim(res)
preds <- log1p(preds) # for prec
dim(preds)
...
preds <- preds[apply(preds, 1, function(x) all(is.finite(x))), ]
inc=diff(range(preds))/500
breaks=seq(quantile(preds, 0.005)-inc, quantile(preds, 0.995)+inc, inc)
ColScheme <- colorRampPalette(rev(brewer.pal(11, "RdYlBu")))(length(breaks)-1)
plot(res, col = ColScheme)

# save as png
png(paste0(results_folder, "dec_full_map.png"), width = 1000, height = 600)
plot(res, col = ColScheme)
dev.off()
```
...
data_wt = xr.concat(datasets[0:3], dim="time")
data_sm = xr.concat(datasets[6:9], dim="time")
...
maps = [lag1_map_wt, lag1_map_sm]
titles = ["Winter", "Summer"]

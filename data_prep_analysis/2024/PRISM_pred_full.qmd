...
era5 = torch.load("../data/era5_tiles_346.pt")
dem = torch.load("../data/dem_tiles_346.pt")
prism = torch.load("../data/prism_tiles_346.pt")
...
```{python}
lr_test = era5[:8,...].unsqueeze(1).to("cuda")
hr_test = prism[:8,...].unsqueeze(1).to("cuda")
dem_test = dem[:8,...].unsqueeze(1).to("cuda")

critic = Critic(12, 144, 1, 1, 1).to("cuda")

generator = Generator(12, 144, 1, 1, 1).to("cuda:0")
test = generator(lr_test.to("cuda"),hr_test.to("cuda"))

test2 = critic(hr_test, dem_test, lr_test)

plt.close()
plt.imshow(test[3,0,...].cpu().detach())
plt.show()

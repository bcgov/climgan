...
stn.info.test <- fread(paste(dir, var,"/ppt_eval_stations_10_90.csv", sep="")) #read in
for (j in which(names(stn.info.test)%in%c(month.abb, "Annual"))) stn.info.test[get(names(stn.info.test)[j])==c(-9999), (j):=NA, ] # replace -9999 with NA
...
mae_dt <- data.table(month = month.abb, MAE = NA)
...
for (i in seq_along(months)) {
  ...
  month <- stringr::str_to_title(months[i])
  mae_dt$MAE[mae_dt$month == month] <- stn.info.test[, mean(abs(stn.info.test[[month]] - terra::extract(db, test.pts)[,2]), na.rm = TRUE)]
}

write.csv(mae_dt, paste0("C:/Users/TGRICE/OneDrive - Government of BC/Documents/GANs/Tirion/Results/foundational_model/prec/Model4/US_MAE.csv"), row.names = FALSE)
```
